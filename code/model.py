# model.py
# Fonction F(t, u) définissant le membre droit du système d'EDO.
# u = [V, N, D, W, B]
...
def F(t, u):
    ...
    # ------------------------------------------------------------------ #
    # Termes intermédiaires (réutilisés dans plusieurs équations)
    # ------------------------------------------------------------------ #

    # Capacités de charge dépendantes de la végétation (formule Monod, eq.6)
    # Plus il y a de végétation, plus le milieu peut supporter d'ongulés.
    kappa_N = K_N * V / (V_star    + V)   # pour wapitis
    kappa_D = K_D * V / (V_starstar + V)  # pour cerfs

    # Prédation des loups sur les wapitis — réponse dépendante du ratio (eq.2)
    # Le taux dépend du ratio W/N et non de N seul : empiriquement validé sur Isle Royale.
    f_WN = c_WN * N / (W + beta * N)

    # Prédation des loups sur les cerfs — réponse Type II proie-dépendante (eq.3)
    f_WD = c_WD * D / (h_WD + D)

    # Prédation des ours sur les wapitis et cerfs — Type II (eq.4)
    f_BN = c_BN * N / (h_BN + N)
    f_BD = c_BD * D / (h_BD + D)

    # Taux total de mise à mort par loup (eq.5) — sert pour la réponse numérique
    Phi_W = f_WN + f_WD
